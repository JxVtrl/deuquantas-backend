...
export class FuncionarioController {
  ...
  async getFuncionarioByCpf(@Param('num_cpf') num_cpf: string) {
    ...
    const funcionario =
      await this.funcionarioService.getFuncionarioByCpf(num_cpf);
    this.logger.log(
  ...
  async createFuncionario(@Body() createFuncionarioDto: CreateFuncionarioDto) {
    this.logger.log(
      `Criando novo funcionário com CPF: ${createFuncionarioDto.num_cpf}`,
    );
    const funcionario =
      await this.funcionarioService.createFuncionario(createFuncionarioDto);
    this.logger.log(
      `Funcionário criado com sucesso. CPF: ${funcionario.num_cpf}`,
    );
    return funcionario;
  }
}
