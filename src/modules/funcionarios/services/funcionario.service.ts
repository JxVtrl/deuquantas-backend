...
export class FuncionarioService {
  ...
  async getAllFuncionarios(): Promise<Funcionario[]> {
    ...
    this.logger.log(`Busca concluída. Total de funcionários encontrados: ${funcionarios.length}`);
    return funcionarios;
  }
  ...
  async getFuncionarioByCpf(num_cpf: string): Promise<Funcionario | null> {
    ...
    const funcionario = await this.funcionarioRepository.findOne({ where: { num_cpf } });
  ...
  async createFuncionario(dto: CreateFuncionarioDto): Promise<Funcionario> {
    this.logger.log(`Iniciando criação de novo funcionário com CPF: ${dto.num_cpf}`);
    const newFuncionario = this.funcionarioRepository.create(dto);
    const savedFuncionario = await this.funcionarioRepository.save(newFuncionario);
    this.logger.log(`Funcionário criado com sucesso. CPF: ${savedFuncionario.num_cpf}`);
