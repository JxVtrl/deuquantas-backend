...
export class SolicitacaoMesaService {
  ...
  async getSolicitacoesByEstabelecimento(cnpj: string): Promise<SolicitacaoMesa[]> {
    this.logger.log(`[DEBUG] Iniciando busca de solicitações no serviço para CNPJ: ${cnpj}`);
    try {
      if (!cnpj) {
        throw new BadRequestException('CNPJ é obrigatório');
      }

      const solicitacoes = await this.solicitacaoMesaRepository.findPendentesByEstabelecimento(cnpj);
      this.logger.log(`[DEBUG] Solicitações encontradas no banco: ${JSON.stringify(solicitacoes)}`);
      return solicitacoes;
    } catch (error) {
      this.logger.error(`[DEBUG] Erro ao buscar solicitações no serviço para CNPJ ${cnpj}:`, error);
  ...
  async getSolicitacaoById(id: string): Promise<SolicitacaoMesa | null> {
    ...
    try {
      ...
      this.logger.log(`[DEBUG] Solicitação encontrada: ${JSON.stringify(solicitacao)}`);
      return solicitacao;
    } catch (error) {
      this.logger.error(`[DEBUG] Erro ao buscar solicitação por ID ${id}:`, error);
  ...
  async solicitarMesa(solicitacao: SolicitacaoMesaDto) {
    try {
      this.logger.log(`[DEBUG] Iniciando solicitação de mesa: ${JSON.stringify(solicitacao)}`);
      
      // Validações
      if (!solicitacao.num_cnpj || !solicitacao.numMesa || !solicitacao.clienteId) {
        this.logger.error('[DEBUG] Dados incompletos:', { solicitacao });
        throw new BadRequestException('Dados incompletos para solicitação de mesa');
      }
      ...
      if (solicitacaoExistente) {
        this.logger.error('[DEBUG] Solicitação pendente já existe:', { solicitacaoExistente });
        throw new BadRequestException('Já existe uma solicitação pendente para esta mesa');
      }
      ...
      this.logger.log(`[DEBUG] Solicitação criada com sucesso: ${JSON.stringify(solicitacaoSalva)}`);
      
      // Notificar o estabelecimento via socket
      const room = `estabelecimento:${solicitacao.num_cnpj}`;
      this.logger.log(`[DEBUG] Tentando notificar estabelecimento na sala ${room}`);
      ...
      this.logger.log(`[DEBUG] IDs dos clientes na sala: ${Array.from(sockets).join(', ')}`);
      ...
      this.logger.error(`[DEBUG] Erro ao criar solicitação: ${error.message}`, error.stack);
      if (error instanceof BadRequestException || error instanceof NotFoundException) {
        throw error;
      }
