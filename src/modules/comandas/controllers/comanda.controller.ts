...
export class ComandaController {
  ...
  async getComandaByCpf(@Param('num_cpf') num_cpf: string) {
    ...
    this.logger.log(`Comanda ${comanda ? 'encontrada' : 'não encontrada'} para o CPF: ${num_cpf}`);
    return comanda;
  }
  ...
  async getTestComandaByCpf(@Param('num_cpf') num_cpf: string) {
    ...
    this.logger.log(`Comanda de teste ${comanda ? 'encontrada' : 'não encontrada'} para o CPF: ${num_cpf}`);
    return comanda;
  }

  @Post()
  async createComanda(@Body() createComandaDto: CreateComandaDto) {
    this.logger.log(`Criando nova comanda para o CPF: ${createComandaDto.num_cpf}`);
    const comanda = await this.comandaService.createComanda(createComandaDto);
    this.logger.log(`Comanda criada com sucesso para o CPF: ${createComandaDto.num_cpf}`);
    return comanda;
  }
}
